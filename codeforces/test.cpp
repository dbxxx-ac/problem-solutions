...
int main() {
  ...
  dijkstra();
  for(int i = 1; i <= m; i++) {
      if(tag[i]||index[pr[u[i]]]==index[pr[v[i]]]) continue;
      if(index[pr[u[i]]]>index[pr[v[i]]])std::swap(u[i],v[i]);
      long long val = pre[u[i]] + suf[v[i]] + w[i];
      tr.modify(1, P.size(), index[pr[u[i]]] + 1, index[pr[v[i]]], 1, val);
    }
  for(int i = 1; i <= q; i++) {
    int t, x;
    scanf("%d%d", &t, &x);
